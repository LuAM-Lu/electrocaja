...
const VentaProcesandoModal = forwardRef(({ 
  ...
  useEffect(() => {
    ...
    const pasosIniciales = [
      ...
      { id: 'procesando', nombre: 'Procesando venta', icon: ShoppingCart, completado: false }
    ];

    if (opcionesProcesamiento?.generarFactura) {
      pasosIniciales.push({ id: 'factura', nombre: 'Generando factura', icon: FileText, completado: false });
      pasosIniciales.push({ id: 'imprimir', nombre: 'Enviando a impresora', icon: Printer, completado: false });
    }

    if (opcionesProcesamiento?.imprimirRecibo) {
      pasosIniciales.push({ id: 'pdf', nombre: 'Generando PDF', icon: FileText, completado: false });
    }
    ...
    if (opcionesProcesamiento?.enviarEmail) {
      pasosIniciales.push({ id: 'email', nombre: 'Enviando email', icon: Mail, completado: false });
    }
  ...
  // ✅ DESHABILITADO: El progreso se controla manualmente desde IngresoModal.jsx
  // Este efecto automático podría causar problemas de sincronización
  // useEffect(() => {
  //   if (!procesando || pasos.length === 0 || avanceManual) return;
  //
  //   const interval = setInterval(() => {
  //     setPasos(prev => {
  //       const nuevos = [...prev];
  //       if (pasoActual < nuevos.length && !nuevos[pasoActual].completado) {
  //         nuevos[pasoActual].completado = true;
  //         setPasoActual(prev => {
  //           const siguiente = prev + 1;
  //           if (siguiente >= nuevos.length) {
  //             setProcesando(false);
  //             setMostrarOpciones(true);
  //             if (onCompletado) {
  //               onCompletado();
  ...
  // }, [procesando, pasoActual, pasos.length, avanceManual]);

  // ✅ NO REDIRIGIR AUTOMÁTICAMENTE - La redirección se maneja desde el componente padre
        ...
        <div className="bg-gradient-to-r from-emerald-600 to-emerald-700 px-8 py-6 text-white">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p id="processing-description" className="text-emerald-100 mt-1 text-sm">
                {procesando ? 'Por favor espera...' : '¡Venta completada exitosamente!'}
              </p>
            </div>
            {procesando && (
          ...
          {procesando ? (
                  ...
                  return (
                    ...
                    >
                      <div className={`flex-shrink-0 ${
                        estaCompletado ? 'text-emerald-400' : esActual ? 'text-blue-400' : 'text-gray-500'
                      }`}>
                        {estaCompletado ? (
                      ...
                      <div className="flex-1">
                        <div className={`font-semibold ${
                          estaCompletado ? 'text-emerald-300' : esActual ? 'text-blue-300' : 'text-gray-400'
                        }`}>
                          {paso.nombre}
                        </div>
                        {esActual && (
            ...
            <>
              ...
              <div className="text-center mb-8">
                ...
                <h3 className="text-3xl font-bold text-white mb-2">¡Venta Completada!</h3>
...
VentaProcesandoModal.displayName = 'VentaProcesandoModal';

export default VentaProcesandoModal;
